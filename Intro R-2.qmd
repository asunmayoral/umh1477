...
1. Guarda en un vector los nombres de las filas ('estados')
1. Transforma en tibble el data.frame ('datos').
1. Asigna 'estados' como nombres de las filas. Comprueba que se guardaron.
1. Visualiza 'datos'.
...
1. Elige 10 filas al azar.
1. Cambia el orden de las filas ordenando por Urbanpop.
...
1. Selecciona las variables 'estados', 'UrbanPop' y 'Murder'.
1. Renombra la variable 'estados' y llámala 'States'.
1. Reordena la variable Murder para que aparezca la primera
```{r}
datos %>%  
...
1. Crea una variable de clasificación, 'urban',  que identifique los estados con un porcentaje de población urbana al menos del 70% (y los que están por debajo).
1. Calcula el número medio de arrestos (por todas las causas) en los estados con al menos el 70% de población urbana y con menos del 70%.
```{r}
...
1. Verifica cómo se llama la columna que identifica los estados, y crea una columna con ese nombre en 'datos', a partir de los nombres de las filas.
1. Combina las dos bases de datos, utilizando 'datos' como principal.
1. Renombra las columnas, 'American.Indian.Alaska.Native' y 'Native.Hawaiian.Other.Pacific.Islander', por 'Native.Amer' y 'Native.Islands' respectivamente.
1. Elimina las dos últimas columnas (Total y Footnotes).
1. Guarda la nueva base de datos en 'datos_ethnic' y visualízala.
...
Algunos valores en las nuevas columnas son "N/A" y ello provoca que las variables sean de tipo carácter.
1. Convierte a factor la variable 'Location'.
1. Convierte a variables numéricas las variables raciales que se han cargado como carácter. Verifica que los valores faltantes se convierten correctamente.
1. Transforma los valores NA a cero.
1. Ordena los estados por la proporción de población blanca.
1. Guárdalo todo en una bd llamada 'datos_clean'.
...
1. Hazlo también con la proporción de población negra.
```{r}
datos_clean %>%
